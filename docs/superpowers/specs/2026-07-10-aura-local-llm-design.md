...
**License note:** Llama 3.2 ships under the Llama Community License (not OSI-free). Fine for personal/dual-boot use; if AuroraOS is ever redistributed publicly, revisit (Qwen2.5-1.5B is Apache-2.0 and a drop-in swap — same GGUF/llama.cpp path).
...
System prompt pins the model to AuroraOS's tool vocabulary and forces JSON tool-calls for actions:

```
You are Aura, the on-device assistant for AuroraOS. You can either answer in prose,
