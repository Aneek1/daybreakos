...
**Files:**
- Modify: `README.md`
- Modify (only if Qwen2.5-1.5B is chosen): `scripts/02-download-sources.sh`, `scripts/10-aurora-shell.sh`, `shell/aurorad.py`, `shell/aura_llm.py`, `shell/aurora-desktop/aurora-shell.c`
...
Also replace `echo "==== downloading Aura model ($AURA_MODEL, ~0.8 GB) ===="` with `echo "==== downloading Aura model ($AURA_MODEL, ~1.0 GB) ===="`.

- [ ] **Step 4 (Qwen2.5-1.5B only): `scripts/10-aurora-shell.sh` line 133**
...
3. In `shell/aurora-desktop/aurora-shell.c` (the "Set up Aura" page), replace `GtkWidget *s = gtk_label_new("Download Aura's on-device AI model (~0.8 GB, "` with `GtkWidget *s = gtk_label_new("Download Aura's on-device AI model (~1.0 GB, "`.
4. In `README.md`:
...
```bash
git add README.md scripts/02-download-sources.sh scripts/10-aurora-shell.sh shell/aurorad.py shell/aura_llm.py shell/aurora-desktop/aurora-shell.c
git commit -F "$HOME/aura-eval/commit-msg.txt"
```
